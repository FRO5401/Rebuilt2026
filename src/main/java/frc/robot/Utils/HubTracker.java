...
public class HubTracker {
  ...
  /**
   * Checks if Timers are between a second of each other or if the custom timer is ahead due to
   * Driverstation relay time
   *
   * @return if the Custom Timer is truly correct return true else false
   */
  protected boolean isTimerCorrect() {
    return (MathHelp.epsilonEquals(getDriverStationTime(), getTimerTime(), 1.05)
  ...
  public Optional<Alliance> getAutoWinner() {
    char gameData =
        DriverStation.getGameSpecificMessage().length() >= 1
  ...
  public Shift getClosestActiveShift() {
    ...
    } else if (getTimeShift(
            getMatchTime() + ((getCurrentShift().startTime - getCurrentShift().endTime) / 2))
        .equals(closestActiveShift)) {
      closestActiveShift =
          getTimeShift(
              getMatchTime() + ((getCurrentShift().startTime - getCurrentShift().endTime) / 2));
    } else {
  ...
  public boolean willShotsCount(double tof) {
    ...
    // Time counts down
    return (getMatchTime() - tof <= getClosestActiveShift().startTime)
        && (getMatchTime() - tof >= getClosestActiveShift().endTime - 3);
  }
