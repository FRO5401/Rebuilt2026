...
public class MathHelp {
      ...
      double numerator =
          targetDistance.in(Meters)
              * Math.sqrt(
                  9.8
                      / (2
                          * (Math.tan(MathConstants.LAUNCH_ANGLE.in(Radians))
                                  * targetDistance.in(Meters)
  ...
  public static AngularVelocity findFlyWheelRPM(LinearVelocity flywheelVelocity) {
    ...
    if (Robot.isSimulation()) {
      Logger.recordOutput(
          "MathHelp/Flywheel RPM",
          (flywheelVelocity.in(MetersPerSecond))
  ...
  public static Time findTOF(Transform2d targDistance) {
    double a = -4.9;
    double b =
        findFlyWheelVelocity(targDistance).in(MetersPerSecond)
  ...
  public static Distance findDistance(Transform2d vector) {
    if (vector != null) {
      Logger.recordOutput(
          "Distance", Math.sqrt(Math.pow(vector.getX(), 2) + Math.pow(vector.getY(), 2)));
  ...
  public static double RPMtoVelocity(double RPM) {
    Logger.recordOutput(
        "MathHelp/Ball Velocity",
        .43 * (RPM * MathConstants.FLY_WHEEL_DIAMETER.baseUnitMagnitude() * Math.PI));
