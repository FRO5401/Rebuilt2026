...
public class Rectangle {
  ...
  public static class RectangleBounds implements Zone {
    ...
    public RectangleBounds getMirrorX() {
      return new RectangleBounds(
          FieldZones.FIELD_LENGTH.in(Meters) - xMax,
          FieldZones.FIELD_LENGTH.in(Meters) - xMin,
          yMin,
          yMax);
    }
    ...
    public RectangleBounds getMirrorY() {
      return new RectangleBounds(
          xMin,
          xMax,
          FieldZones.FIELD_WIDTH.in(Meters) - yMax,
          FieldZones.FIELD_WIDTH.in(Meters) - yMin);
    }
  ...
  public static class PredictiveRectangleX extends RectangleBounds implements PredictiveZone {
    ...
    protected boolean willBeWithin(Translation2d robotPose, ChassisSpeeds velocity, Time time) {
      return isWithinY(robotPose)
          && (checkFutureX(robotPose, velocity, time) || isWithinX(robotPose));
    }

    protected boolean checkFutureX(Translation2d robotPose, ChassisSpeeds velocity, Time time) {
      return (robotPose.getX() < xMin
              && robotPose.getX() + velocity.vxMetersPerSecond * time.in(Seconds) > xMin)
          || (robotPose.getX() > xMax
              && robotPose.getX() + velocity.vxMetersPerSecond * time.in(Seconds) < xMax);
    }
  }
}
