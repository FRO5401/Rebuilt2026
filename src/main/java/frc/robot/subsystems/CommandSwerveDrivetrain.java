...
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem {
  ...
  private final SwerveRequest.FieldCentric robotFieldCentric =
  ...
  public CommandSwerveDrivetrain(
    ...
    backLeftPoseEstimator =
        new PhotonPoseEstimator(
            VisionConstants.APRIL_TAG_FIELD_LAYOUT, VisionConstants.BACK_LEFT_CAMERA_POSE);
  ...
  public ChassisSpeeds findChassisSpeeds() {
    ...
    return new ChassisSpeeds(
        (pose.getX() - lastPose.getX()) / filteredTimeDifference,
  ...
  public void getEstimatedGlobalPose(
      PhotonPoseEstimator poseEstimator, PhotonCamera camera, List<PhotonPipelineResult> results) {
      ...
      if (targets.size() == 1) {
        if (targets.get(0).poseAmbiguity < .1) {
          addVisionMeasurement(
              poseEstimator
                  .estimateAverageBestTargetsPose(results.get(0))
                  .get()
                  .estimatedPose
                  .toPose2d(),
          ...
          && poseEstimator.estimateCoprocMultiTagPose(results.get(0)).isPresent()) {
        addVisionMeasurement(
